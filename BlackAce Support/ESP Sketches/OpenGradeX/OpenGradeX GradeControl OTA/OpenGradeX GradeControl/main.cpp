...
#define DIR1_RL_ENABLE   40 //PD4 //Dir1 for Cytron Dir, Both L and R enable for IBT2
#define PWM1_LPWM  58  //PD3 //PWM1 for Cytron PWM, Left PWM for IBT2
...
//bool buttonState = HIGH; // Assume the button is not pressed
...
float Kd=160; //Mine was 2800 //max servo andle
...
bool SetupGradeControlModule()
{
 ...
 // pinMode(LED33, OUTPUT);  // Initialize the LED33 pin as an output
...

{
  ...
  if (isAutoActive && isCutting && isOGXConnected){    
   ...
   // PID_d = Kd*((delta_error - delta_previous_error)/LOOP_TIME);// calculate the d error
    ...
    if(-b_deadband < delta_error && delta_error < b_deadband){  // 3 cm deadband for i kad nestaiga
    ...
    if (b_deltaDir == 1){ // Delta is Positive need to lower IMP RETRACT
      ...
      pwmDrive = map(PID_total, 0.0, -2048, 89 , 0);
//pwmDrive = map(PID_total, 0.0, -2300, 1500 , 0);

   //   ledcWrite(PWM_Channel2, pwmDrive);
      digitalWrite(DIR1_RL_ENABLE, HIGH);
// servoPosition = map(analogOutput1, retMin, 2048, 0, 180); //65472
      ...
      pwmDrive = map(PID_total, 0.0, 2048, 90 , 180);
     //       pwmDrive = map(PID_total, 0.0, 2300, 2400 , 1501);

 //servoPosition = map(analogOutput1, retMin, 2048, 0, 180); //65472

//servoPositionA = map(analogOutput1, 2048, extMax, 0, 180);
  ...
  //    ledcWrite(PWM_Channel2, pwmDrive);
   ...
   //   voltage2 =((double)analogOutput2/4096) * 5.0;
    ...
    //digitalWrite(DIR1_RL_ENABLE, HIGH);
   // myservo.writeMicroseconds(analogOutput1);  // set servo to mid-point
///////////myservo.write(pwmDrive);

  
   // int servoPosition = map(analogOutput1, retMin, extMax, 0, 180);

///////////
//if (analogOutput1> 2048) {
servoPositionA = map(analogOutput1, 2048, extMax, 180, Kd);
//servoPosition = 0;
//  }
//else{
 servoPosition = map(analogOutput1, retMin, 2048, Kd, 180); //65472
 //servoPositionA = 0;
 ////// }
...
//ledcWrite(PWM_Channel2, servoPosition);



     // voltage2 =((map(PID_total, -2300, 2300, 0 , 1000))/1000) * 5.0;

   //   voltage2 =((double)analogOutput2/4096) * 5.0;
    //  voltage2 =((double)servoPosition/180) * 5.0;

 // if (analogOutput1> 2048) {
   //   voltage2 =(((double)servoPositionA/180) * 2.5)+2.5;
      voltage2 =(map(servoPositionA, 0 ,180, 500, 250))/100.0;



//}
//else{
    //  voltage2 =(((double)servoPosition/180) * 2.5);
      voltage2 =(map(servoPosition, 0, 180, 0 , 250))/100.0;
    ...
    analogOutput2 = VALVE_FLOAT;
               ...
               myservoup.write(180); // Set the servo to a neutral position (90 degrees)  
...
{ 
  switch (_header){
    case DATA_HEADER:
      ...
      UdpGradeControl.print(",");    
      UdpGradeControl.print(voltage2);  // 2  
   ...
   /*  
    ...
    Serial.println("volt2  " + String(voltage2 ));
...
bool  SWIFIData()  // Read Udp Data if Available  
{
 wifiStrength = getWifiStrength(); // Get WiFi signal strength
  ...
  brightness = map(wifiStrength, -100, -40, 0, 150);
...
void ConnectToOGX() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    //  digitalWrite(LED33, 50); // Set LED brightness using PWM
